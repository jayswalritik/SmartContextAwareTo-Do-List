...
public class MainActivity2 extends AppCompatActivity {
    ...
    private boolean isTutorialRunning = true;
    private AnimatorSet swipeAnimatorSet;
    private ObjectAnimator blinkAnimator;
    private View touchableItem = null;
    private View animatedItemView = null;

    private boolean suppressFocusAfterTutorial = false;

    private TapTargetView tapTargetView; // Make sure this is declared in your MainActivity2


    private TaskAdapter.TaskActionListener taskActionListener;
    private RecyclerView.OnItemTouchListener tutorialTouchListener = null;
    ...
    private TapTargetView editTutorialView;
    private View overlayView;
    private TapTargetView activeTargetView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DrawerLayout drawerLayout = findViewById(R.id.drawer_layout);  // Initialize drawerLayout early
        if (getIntent() != null) {
            onNewIntent(getIntent()); // Forward initial intent
        }
        setupWindowInsets();
        initViews();
        initSearchBar();
        ...
        currentCategory = "All"; // <== Set this early
        loadTasksForCategory(currentCategory);
        setSelectedCategoryButton(currentCategory);  // Optional: visually select "All"
        ...
        navigationView.setNavigationItemSelectedListener(item -> {
            ...
            if (id == R.id.nav_home) {
                if (!(MainActivity2.this instanceof MainActivity2)) {
                ...
                Intent intent1 = new Intent(MainActivity2.this, SavedLocationsActivity.class);
                startActivity(intent1);
            } else if (id == R.id.nav_archive) {
                Intent intent1 = new Intent(MainActivity2.this, ArchiveTasksActivity.class);
                startActivity(intent1);
            } else if (id == R.id.nav_feedback) {
                Intent intent1 = new Intent(MainActivity2.this, FeedbackActivity.class);
                startActivity(intent1);
            } else if (id == R.id.nav_tutorial) {
                Intent intent1 = new Intent(MainActivity2.this, TutorialActivity.class);
                startActivity(intent1);
            }
    ...
    private void setupRecyclerView() {
        taskActionListener = new TaskAdapter.TaskActionListener() {
            public void onView(Task task) {
                if (!isSelectionMode) {
                    Intent intent = new Intent(MainActivity2.this, TaskDetailsActivity.class);
            ...
            public void onEdit(Task task) {
                ...
                Intent intent = new Intent(MainActivity2.this, TaskDescription.class);
                intent.putExtra("task_id", task.id);
    ...
    private void loadTasksForCategoryAndStatus(String category, String status) {
        ...
        // 🆕 Update any repeating "pending" task whose datetime has passed
        for (Task task : rawTasks) {
            updateRepeatingTaskIfPast(task); // <-- defined separately
        }
        ...
        for (Task task : rawTasks) {
            ...
            try {
                ...
                if (taskDate != null && taskDate.before(todayStart) && !"completed".equalsIgnoreCase(taskStatus)) {
                    isOverdue = true;
                }
    ...
    private void updateRepeatingTaskIfPast(Task task) {
        if (!"pending".equalsIgnoreCase(task.getTaskStatus())) return;
        if (task.getRepeat() == null || task.getRepeat().equalsIgnoreCase("None")) return;
        ...
        try {
            ...
            if (task.getDate() != null)
                taskTime.setTime(sdfDate.parse(task.getDate()));
            else
                return;

            if (task.getTime() != null && !task.getTime().isEmpty()) {
                Date timePart = sdfTime.parse(task.getTime());
                Calendar temp = Calendar.getInstance();
                temp.setTime(timePart);
                taskTime.set(Calendar.HOUR_OF_DAY, temp.get(Calendar.HOUR_OF_DAY));
                taskTime.set(Calendar.MINUTE, temp.get(Calendar.MINUTE));
            } else {
                taskTime.set(Calendar.HOUR_OF_DAY, 23);
                taskTime.set(Calendar.MINUTE, 59);
            }

            Calendar now = Calendar.getInstance();

            if (now.after(taskTime)) {
                // 🔍 Extract repeat interval and unit from string like "Every 50 minutes"
                String repeatText = task.getRepeat().toLowerCase();
                int interval = 1;
                String unit = "";

                Matcher matcher = Pattern.compile("every\\s+(\\d+)\\s+(minute|hour|day|week|month)s?").matcher(repeatText);
                if (matcher.find()) {
                    interval = Integer.parseInt(matcher.group(1));
                    unit = matcher.group(2);
                } else {
                    // fallback for fixed patterns
                    switch (repeatText) {
                ...
                // ⏱ Apply repetition
                switch (unit) {
                    case "minute": taskTime.add(Calendar.MINUTE, interval); break;
                    case "hour": taskTime.add(Calendar.HOUR_OF_DAY, interval); break;
                    case "day": taskTime.add(Calendar.DAY_OF_YEAR, interval); break;
                    case "week": taskTime.add(Calendar.WEEK_OF_YEAR, interval); break;
                    case "month": taskTime.add(Calendar.MONTH, interval); break;
                    default: return;
                }

                String newDate = sdfDate.format(taskTime.getTime());
                String newTime = sdfTime.format(taskTime.getTime());
    ...
    private void setupStatusTabs() {
        ...
        statusTabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                ...
                // Always show/hide action row based on selection mode
                if (isSelectionMode) {
                    actionRow.setVisibility(View.VISIBLE);
                    homePageRow.setVisibility(View.GONE);
                } else {
                ...
                // ✅ Update the radio button state for current tab
    ...
    private boolean matchesStatus(Task task, String statusTab) {
        ...
        switch (statusTab.toLowerCase()) {
            ...
            case "overdue":
                try {
                    Date taskDate = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).parse(task.getDate());
                    ...
                    return status.equals("pending") && taskDate.before(todayStart);
                } catch (Exception e) {
                    return false;
                }
            default:
        ...
        ItemTouchHelper.SimpleCallback callback = new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder,
                ...
                if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE && isCurrentlyActive && dX < 0 && !isSelectionMode) {
                    ...
                    Drawable deleteIcon = ContextCompat.getDrawable(MainActivity2.this, R.drawable.deleteicon);
                    if (deleteIcon != null) {
        ...
        tapTargetView = TapTargetView.showFor(
                ...
                new TapTargetView.Listener() {
                    ...
                    public void onTargetCancel(TapTargetView view) {
                        ...
                        Toast.makeText(MainActivity2.this, "Tutorial cancelled.", Toast.LENGTH_SHORT).show();
                    }
                }
        );
    ...
    private void showViewTaskTutorial() {
        recyclerView.post(() -> {
            ...
            if (viewHolder != null && adapter.getItem(0) instanceof TaskAdapter.TaskItem) {
                ...
                Toast.makeText(MainActivity2.this, "No task available to show the tutorial.", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void showSelectTaskTutorial() {
        recyclerView.post(() -> {
            ...
            if (viewHolder1 != null && adapter.getItem(0) instanceof TaskAdapter.TaskItem) {
                ...
                Toast.makeText(MainActivity2.this, "Not enough tasks for tutorial.", Toast.LENGTH_SHORT).show();
            }
        });
    }
            ...
            if (viewHolder != null) {
                ...
                tapTargetView = TapTargetView.showFor(
                        ...
                        new TapTargetView.Listener() {
                            ...
                            public void onTargetDismissed(TapTargetView view, boolean userInitiated) {
                                super.onTargetDismissed(view, userInitiated);
                                Toast.makeText(MainActivity2.this, "Tutorial complete!", Toast.LENGTH_SHORT).show();
                                suppressSearchBarFocus(); // ⬅️ Prevent search bar focus
                                tapTargetView = null;
                                enableTaskCheckboxes();
                            }
